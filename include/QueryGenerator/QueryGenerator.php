...
class QueryGenerator {
	...
	public function getQuery() {
		...
		if(empty($this->query)) {
			...
			//08/08/23 Leo
			//$meta = $this->getMeta($module);
			$baseTable_ = $this->meta->getEntityBaseTable();
			$moduleTableIndexList_ = $this->meta->getEntityTableIndexList();
			$baseTableIndex_ = $moduleTableIndexList_[$baseTable_];
			//$log->info("base $baseTable_");

			$this->query = $query;
			$log->info("la consulta que se va a ejecutar es: ");
			//$where_ = "WHERE vtiger_crmentity.deleted=0 AND ". $baseTable_.".".$baseTableIndex_ ." > 0";
			//$where2 = "WHERE vtiger_crmentity.deleted=0 AND vtiger_modcomments.modcommentsid > 0";
			//$log->info($where_);
			//$log->info($where2);
			//Si entro a la lista general sin filtros uso el hint
			if(HINT_ACTIVO){ //Variable en config-ludere para el uso de hint. true si se usa LR 22/08/23
				if (strpos($query, "WHERE vtiger_crmentity.deleted=0 AND ". $baseTable_.".".$baseTableIndex_." > 0") != false) {
					if (strpos($query, "vtiger_modcomments") == false ) {
						if (strpos($query, "vtiger_consultasweb") == false ) {
							if (strpos($query, "vtiger_atencionpresencial") == false ) {
								if (strpos($query, "vtiger_atencionesweb") == false ) {
									if (strpos($query, "vtiger_parametrizaciones") == false ) {
										if (strpos($query, "vtiger_topics") == false ) {
											$log->info("entro a listas sin filtro");
											$query = str_replace("INNER JOIN vtiger_crmentity ON",
					                     	"INNER JOIN vtiger_crmentity USE INDEX (modifiedtime) ON", 
					                     	$query);
										}
									}
								}
				...
				/*if (strpos($query, "vtiger_users.userlabel") == false) {
					$quitar = "LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id";
	    			$quitar2 = "LEFT JOIN vtiger_groups ON vtiger_crmentity.smownerid = vtiger_groups.groupid";
	    			$query = str_replace($quitar, "", $query);
					$query = str_replace($quitar2, "", $query);
					$log->info($query);
				}
				else{
					$query = str_replace("INNER JOIN vtiger_crmentity ON", 
			...
			//$log->info("la consulta que se ejecuta en el else es: ");
			//$log->info( $this->query);
			return $this->query;
		}
	}
