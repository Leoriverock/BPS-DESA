...
function combo($aportacion){
		global $log;
		$log->info("showme ".$aportacion);
		if($aportacion == '1'){
			$tipoaportacion = '1 - Industria y comercio';
		}
		if($aportacion == '2'){
			$tipoaportacion = '2 – Civil';
		}
		if($aportacion == '3'){
			$tipoaportacion = '3 – Rural';
		}
		if($aportacion == '4'){
			$tipoaportacion = '4 – Construcción';
		}
		if($aportacion == '5'){
			$tipoaportacion = '5 – Notarial';
		}
		if($aportacion == '6'){
			$tipoaportacion = '6 – Bancaria';
		}
		if($aportacion == '7'){
			$tipoaportacion = '7 - Trabajadores a domicilio';
		}
		if($aportacion == '11'){
			$tipoaportacion = '11 - Servicios personales no profesionales y profesionales';
		}
		if($aportacion == '48'){
			$tipoaportacion = '48 - Servicio doméstico';
		}
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module=false)
{
	...
	if($user_array == null)
	{
		...
		if (empty($status)) {
				...
				{
					$log->debug("Sharing is Public. All vtiger_users should be listed2");
					...
					$log->info("el modulo es $module");
					//Defecto #155191 LR 15/09/23 PARA EL CASO DE CW SOLO DEBE LISTAR USUARIOS ASOC AL GRUPO DEL SUPERVISOR
					if($module == 'ConsultasWeb'){
						$query = "SELECT DISTINCT id, user_name,first_name,last_name,userlabel 
								FROM vtiger_users u
								INNER JOIN vtiger_users2group ug ON u.id = ug.userid
								WHERE STATUS= ?
								AND (user_name != 'admin' OR is_owner=1) 
								AND ug.groupid IN (SELECT G.groupid
								FROM vtiger_groups G
								INNER JOIN vtiger_users2group UG ON G.groupid = UG.groupid
								WHERE UG.userid = ?)";
					$params = array($status,$usuarioLogueado );
					}
				}
		}
...
{
	global $log;
	$log->debug("Entering get_user_array11(".$add_blank.",". $status.",".$assigned_user.",".$private.") method ...");
	global $current_user;
	if(isset($current_user) && $current_user->id != '')
	{
		require('user_privileges/sharing_privileges_'.$current_user->id.'.php');
	...
	if($group_array == null)
	{
		...
		if($private != 'private' && $module == 'ConsultasWeb' ){
			$query .= " WHERE type = 'mail' ";
		}
...
function decodeTildes($cadena){
	return str_replace(array('&aacute;','&eacute;','&iacute;','&oacute;','&uacute;','&ntilde;','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;','&Ntilde;', '&deg;', '&ordm;', '&quot;', '&acute;'), array('á','é','í','ó','ú','ñ','Á','É','Í','Ó','Ú','Ñ', '°', 'º', '"', '´'), $cadena);    
}
