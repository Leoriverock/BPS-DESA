...
class Users extends CRMEntity {
	...
	function insertIntoEntityTable($table_name, $module, $fileid='') {
		...
		if ($insertion_mode == 'edit') {
			$log->info("voy a editar preferencias de usuario");
		
			// Encuentra la posición de "us_grupopref=?" en la cadena $update
			$usGrupoprefPosition = array_search('us_grupopref=?', explode(', ', $update));
		
			// Si se encuentra "us_grupopref=?", lo eliminamos
			if ($usGrupoprefPosition !== false) {
				$update = str_replace('us_grupopref=?, ', '', $update);
		
				// Elimina el elemento correspondiente de $update_params
				array_splice($update_params, $usGrupoprefPosition, 1);
			}
		
			// Verifica si $update aún contiene contenido después de eliminar "us_grupopref=?"
			if (trim($update) != '') {
	...
	*/
	public function setUserPreferences($requestArray) {
		global $adb,$log;
		$log->info("estoy en setUserPreferences");
		$updateData = array();
				...
				foreach($blockInstance as $fieldInstance) {
					...
					if($fieldName == 'status') {
						...
						foreach($allRoles as $role) {
							if(strtolower($fieldValue) == strtolower(html_entity_decode($role->getName()))) {
								$roleId = $role->getId();
								break;
							}
						}
