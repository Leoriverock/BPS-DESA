...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	public function getAtencionActiva(){
		global $adb;
		$sql = "SELECT crmid FROM vtiger_atencionesweb a INNER JOIN vtiger_crmentity e ON e.crmid = a.atencioneswebid AND e.deleted = 0 WHERE a.aw_estado = ? AND e.smownerid = ?";
		$rs = $adb->pquery($sql, array('Asignado', $this->getId()));
		if($adb->num_rows($rs) > 0){
			return $rs->fields['crmid'];
		}
		return false;
	}
	public function getCallinGroups(){
		global $adb;
		$currentUserId = Users_Record_Model::getCurrentUserModel()->get('id');
		$sql = "SELECT 1 FROM vtiger_groups 
				JOIN vtiger_users2group ON vtiger_users2group.groupid = vtiger_groups.groupid
				WHERE TYPE LIKE '%Llamadas%' 
				AND userid = ? ";
		$rs = $adb->pquery($sql, array($currentUserId));
		if($adb->num_rows($rs) > 0){
			return true;
		}
		return false;
	...
	public function getAtencionPActiva(){
		global $adb;
		$sql = "SELECT crmid FROM vtiger_atencionpresencial a INNER JOIN vtiger_crmentity e ON e.crmid = a.atencionpresencialid AND e.deleted = 0 WHERE a.ap_estado = ? AND e.smownerid = ?"; //AND DATE_FORMAT(ap_fechacomienzo, '%Y-%m-%d') = CURDATE()
		$rs = $adb->pquery($sql, array('en Proceso', $this->getId()));
		if($adb->num_rows($rs) > 0){
			return $rs->fields['crmid'];
		}
		return false;
	...
	public function getConectado(){
		global $adb, $log;
		$log->info("entreo a getConectado");
		$user_query = "SELECT equipo FROM vtiger_users WHERE id = ? AND deleted = 0 ";
		$result = $adb->pquery($user_query,array($this->getId()) );

		if($adb->num_rows($result) > 0) $conectado = $adb->query_result($result, 0, 'equipo');
	...
	public function getEmailActiva(){
		global $adb;
		$sql = "SELECT aw_de FROM vtiger_atencionesweb a INNER JOIN vtiger_crmentity e ON e.crmid = a.atencioneswebid AND e.deleted = 0 WHERE a.aw_estado = ? AND e.smownerid = ?";
		$rs = $adb->pquery($sql, array('Asignado', $this->getId()));
		if($adb->num_rows($rs) > 0){
			return $rs->fields['aw_de'];
		}
		return false;
	}

	public function getDatosAPActiva(){
		global $adb;
		$sql = "SELECT ap_numero 
				FROM vtiger_atencionpresencial a 
				INNER JOIN vtiger_crmentity e ON e.crmid = a.atencionpresencialid AND e.deleted = 0 
				WHERE a.ap_estado = ? 
				AND e.smownerid = ?
				ORDER BY ap_fechacomienzo ASC LIMIT 1";
				//Falta agregar filtro para la hora y fecha de la atencion
		$rs = $adb->pquery($sql, array('Asignado', $this->getId()));
		if($adb->num_rows($rs) > 0){
			return $rs->fields['ap_numero'];
		}
		return false;
	}

	public function getGroupsPreferencesOption($type = 'All'){
		global $adb;
		$result = array();
		$grupos = $this->getUserGroups($this->getId());
		foreach($grupos as $id){
			$model = Settings_Groups_Record_Model::getInstance($id);
			if($model){
				if($type ==  'All'){
					$obj = null;
					$obj->id = $id;
					$obj->label = $model->getName();
					$result[] = $obj;
				}else{
					if(in_array($type, explode(' |##| ', $model->getType()))){
						$obj = null;
						$obj->id = $id;
						$obj->label = $model->getName();
						$result[] = $obj;
	...
	public function tieneTickets($id){
		global $adb,$log;
		$log->info("estoy en tieneTickets $awid");
		/*$sql = "SELECT  FROM vtiger_crmentityrel r
				...
				WHERE ? IN (r.crmid,r.relcrmid) ";*/
		//Si obtengo 1 en alguna consulta hay relacion o comentario
		$sql = "SELECT 1 FROM DUAL WHERE  EXISTS (
  				SELECT 1 
			  	FROM vtiger_crmentityrel r 
			  	INNER JOIN vtiger_crmentity e1 ON (e1.crmid = r.relcrmid OR e1.crmid = r.crmid) 
			    AND setype = 'HelpDesk' AND e1.deleted = 0 
			  	WHERE ? =  r.crmid or ? = r.relcrmid
			  	UNION
			  	SELECT 1 FROM vtiger_modcomments WHERE callsid = ?
				)";
		$rs = $adb->pquery($sql, array($id,$id,$id));
	...
	public function tienePermiso(){
        global $adb, $log;
        $AtencionesWeb = Vtiger_Module_Model::getInstance('AtencionesWeb');
       
        if(!$AtencionesWeb) return false;
        $awid = $AtencionesWeb->getId();
        $log->info("Estoy en AtencionesWeb_Record_Model");
        $log->info($awid);
        ...
        $currentUserId = Users_Record_Model::getCurrentUserModel()->get('id');
            ...
            $permiso = $adb->query_result($adb->query("SELECT pu.permission
                                        ...
                                        WHERE ur.userid = $currentUserId AND pu.tabid = $awid AND am.actionname = 'btnSiguiente'"), 0, 
                                            'permission');
            if ($permiso == 0) {
                return true;
            } else {
    ...
    public function tienePermisoAP(){
        global $adb, $log;
        $AtencionPresencial = Vtiger_Module_Model::getInstance('AtencionPresencial');
       
        if(!$AtencionPresencial) return false;
        $apid = $AtencionPresencial->getId();
        $log->info("Estoy en tienePermisoAP");
        $log->info($apid);
        ...
        $currentUserId = Users_Record_Model::getCurrentUserModel()->get('id');
            ...
            $permiso = $adb->query_result($adb->query("SELECT pu.permission
                                        ...
                                        INNER JOIN vtiger_user2role ur ON ur.roleid = rp.roleid
                                        INNER JOIN vtiger_actionmapping am ON am.actionid = pu.activityid
                                        WHERE ur.userid = $currentUserId AND pu.tabid = $apid AND am.actionname = 'btnVerAtencion'"), 0, 
                                            'permission');
            if ($permiso == 0) {
                return true;
            } else {
    ...
    public function tienePermisoPresencial(){
        global $adb, $log;
        $AtencionesWeb = Vtiger_Module_Model::getInstance('AtencionPresencial');
        ...
        $currentUserId = Users_Record_Model::getCurrentUserModel()->get('id');
            ...
            $permiso = $adb->query_result($adb->query("SELECT pu.permission
                                        ...
                                        INNER JOIN vtiger_user2role ur ON ur.roleid = rp.roleid
                                        INNER JOIN vtiger_actionmapping am ON am.actionid = pu.activityid
                                        WHERE ur.userid = $currentUserId AND pu.tabid = $awid AND am.actionname = 'btnAbrirPuesto'"), 0, 
                                            'permission');
            if ($permiso == 0) {
                return true;
            } else {
