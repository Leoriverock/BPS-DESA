...
class HelpDesk_Module_Model extends Vtiger_Module_Model {
	...
	public function getTicketsByStatus($owner, $dateFilter) {
		...
		//Se optimiza consulta LR 13-09-23							
		$result = $db->pquery('SELECT COUNT(*) as count, COALESCE(vtiger_troubletickets.status, "") AS statusvalue 
							FROM vtiger_troubletickets INNER JOIN vtiger_crmentity ON vtiger_troubletickets.ticketid = vtiger_crmentity.crmid AND vtiger_crmentity.deleted=0
							'.Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName()). $ownerSql .' '.$dateFilterSql.
							' LEFT JOIN vtiger_ticketstatus ON vtiger_troubletickets.status = vtiger_ticketstatus.ticketstatus 
							WHERE vtiger_troubletickets.status IN ('.generateQuestionMarks($picklistvaluesmap).') 
							AND vtiger_troubletickets.status <> "Closed"
							GROUP BY statusvalue 
							ORDER BY COALESCE(vtiger_ticketstatus.sortorderid, 0)', $params);
	...
	public static function isRelation( $contribuyente, $empresa, $codigoaportacion ){
		$params = [
			"ticketcontribuyente" => $contribuyente,
			"ticketempresa" => $empresa
		];
		if( !empty( $codigoaportacion ) ){
			$params['ticketcodigoaportacion'] = $codigoaportacion;
		}
		return getDatosEmpresa( $params );
	}
}
