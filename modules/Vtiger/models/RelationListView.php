...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model {
	...
	public function getRelationQuery() {
		...
		if(!empty($relationModel) && $relationModel->get('name') != NULL){
			...
			/*
			Prueba para optimizar consultas 
			*/
			$patron = "/\b\d+\b/";
			$recordID = '';
			...
			//Si la vista relacionada es llamadas
			...
			//Si la vista relacionada es AtencionesWeb
			if (strpos($query, 'vtiger_atencionesweb') !== false) {
    			$query = '';

				$log->info("La query del generador vtiger_atencionesweb: ");
				$query = '	SELECT * FROM (
							SELECT vtiger_atencionesweb.*, vtiger_crmentity.*  FROM vtiger_atencionesweb 
							...
							LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
							LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
 							WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid = '.$recordID[1].' OR vtiger_crmentityrel.relcrmid = '.$recordID[1].')) as dat INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = dat.atencioneswebid 
							'; 
							

			}
			//Si la vista relacionada es vtiger_consultasweb
			if (strpos($query, 'vtiger_consultasweb') !== false) {
    			$query = '';

				$log->info("La query del generador vtiger_consultasweb: ");
				$query = '	SELECT * FROM (
							SELECT vtiger_consultasweb.*, vtiger_crmentity.*  FROM vtiger_consultasweb 
							INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_consultasweb.consultaswebid 
							INNER JOIN vtiger_crmentityrel ON (vtiger_crmentityrel.relcrmid = vtiger_crmentity.crmid) 
							LEFT JOIN vtiger_consultaswebcf ON vtiger_consultaswebcf.consultaswebid = vtiger_consultasweb.consultaswebid 
							LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
							LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
							WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid = '.$recordID[1].' OR vtiger_crmentityrel.relcrmid = '.$recordID[1].' )
							UNION
							...
							LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
							LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
 							WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid = '.$recordID[1].' OR vtiger_crmentityrel.relcrmid = '.$recordID[1].')) as dat INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = dat.consultaswebid 
							'; 
							

			}
			//Si la vista relacionada es vtiger_atencionpresencial
			/*if (strpos($query, 'vtiger_atencionpresencial') !== false) {
    			$query = '';

				$log->info("La query del generador vtiger_atencionpresencial: ");
				$query = '	SELECT * FROM (
							SELECT vtiger_atencionpresencial.*, vtiger_crmentity.*  FROM vtiger_atencionpresencial 
							INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_atencionpresencial.atencionpresencialid 
							INNER JOIN vtiger_crmentityrel ON (vtiger_crmentityrel.relcrmid = vtiger_crmentity.crmid) 
							LEFT JOIN vtiger_atencionpresencialcf ON vtiger_atencionpresencialcf.atencionpresencialid = vtiger_atencionpresencial.atencionpresencialid 
							LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
							LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
							WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid = '.$recordID[1].' OR vtiger_crmentityrel.relcrmid = '.$recordID[1].' )
							UNION
							...
							LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
							LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
 							WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid = '.$recordID[1].' OR vtiger_crmentityrel.relcrmid = '.$recordID[1].')) as dat INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = dat.atencionpresencialid 
							'; 
