...
class Vtiger_Relation_Model extends Vtiger_Base_Model{
	...
	public function getQuery($parentRecord, $actions=false){
		...
		if ($relatedModuleName == "ModComments") {
			...
			if($functionName != 'get_related_list') $query = $focus->$functionName($parentRecord->getId());
			else $query = $focus->$functionName($parentRecord->getId(), $parentModuleModel->getId(), $relatedModuleModel->getId());
		} else {
