...
function addSignature($contents, $fromname)
{
	...
	$sign = nl2br($sign);
	$sign = eliminar_acentos(decodeTildes($sign));
	$log->info("estoy en add signature");
	$log->info($sign);
...
function setMailerProperties($mail, $subject, $contents, $from_email, $from_name, $to_email, $attachment = '', $emailid = '', $module = '', $logo = '')
{
	...
	if ($attachment == "vinculados") {
		//en realidad es el email de la incidencia pero lo paso aca ja
		addAllAttachmentsVinculados($mail, $emailid);
	}
...
/* AGREGA LOS DOCUMENTOS VINCULADOS AL REGISTRO */
function addAllAttachmentsVinculados($mail, $comments)
{
	global $adb, $log, $root_directory;
	$adb->println("Inside the function addAllAttachments");
	...
	foreach ($comments as $commentid) {
		//Retrieve the vtiger_files from database where avoid the file which has been currently selected
		$sql = "SELECT vtiger_attachments.attachmentsid, path, name, storedname FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						WHERE crmid = ?";
		$res = $adb->pquery($sql, array($commentid));
		$count = $adb->num_rows($res);

		for ($i = 0; $i < $count; $i++) {
			$fileid = $adb->query_result($res, $i, 'attachmentsid');
			$filename = decode_html($adb->query_result($res, $i, 'storedname'));
			$name = decode_html($adb->query_result($res, $i, 'name'));
			$filepath = $adb->query_result($res, $i, 'path');
			$filewithpath = $root_directory . $filepath . $fileid . "_" . $filename;
