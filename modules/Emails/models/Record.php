...
class Emails_Record_Model extends Vtiger_Record_Model {
	...
	public function send($addToQueue = false) {
		...
		$replyTo = CORREO_SALIDA_RESPUESTA;//$this->getReplyToEmail();
		$userName = NAME_CORREO_SALIDA_RESPUESTA;

		//$userName = $currentUserModel->getName();
		/*Agregado server name Leo*/
		/*$result = $adb->pquery('SELECT server_username FROM vtiger_systems WHERE server_type=?', array('email'));
			if ($adb->num_rows($result)) {
				$fromName = decode_html($adb->query_result($result, 0, 'server_username'));
			}
			if (!empty($fromEmail)) {
				$userName = ucfirst($fromName);
			}*/

		


		$log->info("nameeeee ".$userName);
		$log->info("replicame!!!!!!! ".$replyTo);
		...
		foreach($toEmailInfo as $selectedId => $selectedEmails) {
			...
			$generatedMessageId = '';
			$mailer->reinitialize();
			$log->info("from email ffffffffff ".$fromEmail);
            ...
            //Se quito firma por defecto - se precarga por js en composeemailform.tpl LR 16/08/23 Defecto #149898
            /*if($this->get('signature') == 'Yes'){
                $mailer->Signature = $currentUserModel->get('signature');
                //Defecto #149898 LR  15/08/23
                $mailer->Signature = utf8_decode($mailer->Signature);
                $log->info("la firma del user es: ");
	...
	function getReplyToEmail() {
		...
		$log->info("getReplyToEmail");
		$log->info($defaultReplyTo);
