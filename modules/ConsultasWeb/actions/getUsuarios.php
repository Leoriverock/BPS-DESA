...
class ConsultasWeb_getUsuarios_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		$sql = "SELECT DISTINCT id, CONCAT(first_name ,' ',last_name) AS nombrecompleto
				FROM vtiger_users
				LEFT JOIN vtiger_users2group ON userid = id AND userid <> ?
				INNER JOIN vtiger_user2role ON vtiger_user2role.userid = id
				WHERE id NOT IN(
						SELECT e.smownerid FROM vtiger_atencionesweb a 
				...
				";/*UNION
				SELECT groupid id, groupname  nombrecompleto
					FROM vtiger_groups
					WHERE groupid NOT IN  (
							?
							)*/
		$result = $adb->pquery($sql,array($usuarioLogueado,'Asignado',$grupo)); //EJECUTIVOS_MAILS -- AND vtiger_user2role.roleid = ?
