...
class ConsultasWeb_CargarModalConsultas_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if($asignado == 0){
			$log->debug("toy en ConsultasWeb_CargarModalConsultas_View usuario: $consultaswebid  ");
		...
		$datos_consultaweb = $adb->pquery($sql, array('Pendiente','Pendiente Agente',$consultaswebid));
		...
		$grupo = $adb->query_result($datos_consultaweb, 0, 'grupo');
		...
		//$consultasweb[] = "";	
		$result = $adb->pquery($consulta,array($personaid,$de_email,$grupo,$temaid,'Pendiente',$empresa,$contribuyente,$id)); 
		...
		$resultado = $adb->pquery($adjunto,array($consultaswebid,1));
