...
class Settings_Groups_Record_Model extends Settings_Vtiger_Record_Model {
	...
	/**
	 * Function to get the types of the group
	 * @return <String>
	 */
	...
	public function save() {
		...
		if ($mode == 'edit') {
			$sql = 'UPDATE vtiger_groups SET groupname=?, description=?, type = ? WHERE groupid=?';
			$params = array($this->getName(), $this->getDescription(), $this->getType(), $groupId);
		} else {
			$sql = 'INSERT INTO vtiger_groups(groupid, groupname, description, type) VALUES (?,?,?,?)';
			$params = array($groupId, $this->getName(), $this->getDescription(), $this->getType());
		}
		...
		//LR SETEO grupopref al usuario cuando el mismo solo pertenece a un grupo de tipo mails
		//Obtengo la lista de usuarios que habia en el grupo
		$newArray = [];
		$membersOld_ = [];

		foreach ($membersOld['Users'] as $key => $value) {
			$newArray[] = $value;
		}
		$log->info("la lista de usuarios que era : ");
		foreach ($newArray as $user) {
			$id = $user->valueMap['id'];
			$membersOld_[] = $id;
			
		}
		$log->info($membersOld_);

		$sql = "SELECT type FROM vtiger_groups WHERE groupid = ?";
		$rs = $adb->pquery($sql, array($groupId));
		$type = $adb->query_result($rs, 0, 'type');
		if (trim($type) == 'mail'){
			
			
			
			$miembrosUnidos = array_diff($members, $membersOld_);
			$miembrosSeFueron = array_diff($membersOld_, $members);

			if (count($miembrosUnidos) > count($miembrosSeFueron)) {
				
				$diferencia = $miembrosUnidos;
				$accion = 'unidos';
				foreach ($diferencia as $elemento) {
					$partes = explode(":", $elemento);
					$userid = end($partes);
					$log->info($userid);
					$sql = "SELECT COUNT(*) AS cant_grupo, g.groupid
								FROM vtiger_users2group ug
								INNER JOIN vtiger_groups g ON g.groupid = ug.groupid AND g.type = ?
								WHERE userid = ? ";
					$res = $adb->pquery($sql,array('mail',$userid));
					$grupoid = $adb->query_result($res, 0, 'groupid');
					if ($res && $adb->num_rows($res) == 1) {
						$log->info("entra al if");
						$log->info($adb->num_rows($res));
						//Seteo grupo de preferencia para el usuario
						$update = "UPDATE vtiger_users SET us_grupopref = ? where id = ?";
						$adb->pquery($update,array($grupoid,$userid));
					}
				}

		
			} elseif (count($miembrosUnidos) < count($miembrosSeFueron)) {
				// Hay más miembros que se han ido
				$diferencia = $miembrosSeFueron;
				$accion = 'se fueron';
				foreach ($diferencia as $elemento) {
					$partes = explode(":", $elemento);
					$userid = end($partes);
				...
				// El número de miembros nuevos y miembros que se fueron es igual
				$diferencia = [];
				$accion = 'sin cambios';
			}
