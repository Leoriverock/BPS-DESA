...
class Vtiger_MailScanner {
	...
	function performScanNow() {
		...
		if (!$connected) {
			return "Error al conectar al servidor de correos";
		}
	 ...
	 * Lookup Account record based on the email given.
	 */
	function LookupAccountAtenciones($email) {
		global $adb;
		if($this->_cachedAccountIds[$email]) {
			$this->log("Reusing Cached Account Id for email: $email");
			return $this->_cachedAccountIds[$email];
		...
		$accountres = $adb->pquery("SELECT accountid
			FROM vtiger_account
			INNER JOIN vtiger_crmentity ON crmid = accountid AND deleted = 0 AND setype = ?
			WHERE accountid in (
			SELECT aw_persona
			FROM vtiger_atencionesweb a
			INNER JOIN vtiger_crmentity e ON a.atencioneswebid = e.crmid AND e.deleted = 0
			WHERE a.aw_de = ? AND a.aw_estado in ('Asignado', 'Pausada'))", array('Accounts', $email));
		if($adb->num_rows($accountres)) {
			$accountid = $adb->query_result($accountres, 0, 'accountid');
		}
		if($accountid) {
	 ...
	 * Get Ticket record information based on subject or id.
	 */
	function LP_GetTicketRecord($matchs, $subjectOrId) {
		require_once('modules/HelpDesk/HelpDesk.php');
		$ticketid = $this->LP_LookupTicket($subjectOrId, $matchs);
		$ticket_focus = false;
		if($ticketid) {
			if($this->_cachedTickets[$ticketid]) {
				$ticket_focus = $this->_cachedTickets[$ticketid];
	 ...
	 * LuderePro - KM
	 * Lookup Ticket record based on the subject or id given.
	 * La modificamos por la forma de obtener el codigo a partir del subject
	 * 2020/04/27
	 */
	function LP_LookupTicket($subjectOrId, $pattern) {
		global $adb;
		$checkTicketId 	= false;
		$matchs 		= array();
		// El patron de los codigos de atencion es ATEXXX 
		$res 			= preg_match("/$pattern\d+/", $subjectOrId,$matchs);
		if($res > 0){
			$ticket_no = $matchs[0];
			$rs = $adb->pquery("SELECT ticketid FROM vtiger_troubletickets WHERE ticket_no = ? ",array($ticket_no));
			if($adb->num_rows($rs)){
				$checkTicketId = $adb->query_result($rs,0,'ticketid');
			}
		}
		// Nothing found?
		if(!$checkTicketId) return false;
