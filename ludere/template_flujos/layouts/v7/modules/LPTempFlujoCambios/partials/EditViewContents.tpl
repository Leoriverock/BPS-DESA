{*valores para los dos selects, origen y destino*}
<input type="hidden" name="campos_valores" value='{Vtiger_Functions::jsonEncode($CAMPOS_VALORES)}' />
{*valores para los dos selects, origen y destino*}
{strip}
								...
								{if $FIELD_MODEL->isEditable() eq true}
									...
									{if $FIELD_MODEL->get('uitype') neq '83'}
										<td class="fieldValue" {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
											{* control para campo tfc_template *}
											{if $FIELD_MODEL->get('label') eq "tfc_template"} 
												<select name="tfc_template" class="inputElement select2" 
												data-fieldname="{$FIELD_MODEL->getFieldName()}" 
												data-fieldtype="picklist" 
												data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
												{foreach from=$TEMPLATES item=TEMPLATE}
													<option value="{$TEMPLATE['lptempflujosid']}" 
													data-modulo="{$TEMPLATE['tf_modulo']}" 
													data-campo="{$TEMPLATE['tf_campo_mod']}" 
													{if $FIELD_MODEL->get('fieldvalue') eq {$TEMPLATE['lptempflujosid']}} selected {/if} >
													{vtranslate($TEMPLATE['tf_nombre'], $MODULE)}
													</option>
												{/foreach}
												</select>
												{* fin control para campo tfc_template *}
											{else}
