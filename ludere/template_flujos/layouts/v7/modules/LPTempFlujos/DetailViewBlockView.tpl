{strip}
	...
	<div class="block">
		<div>
			<h4 class="textOverflowEllipsis" style="display: flex;height: 30px;">
				<span style="margin: auto 0;">
					{vtranslate("Diseño",{$MODULE_NAME})}
				</span>
				<span style="margin: auto 10px;">
					<a href="#" data-toggle="tooltip" title="Mantener el cursor clickeado sobre las uniones para modificar los flujos">
						<span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>
					</a>				
				</span>
				<div class="pull-right" style="margin-left: auto;">
					{* <button  class="btn btn-accent" onclick="cargar_grafo()"><strong>{vtranslate('Recargar', $MODULE)}</strong></button> *}
					<button  class="btn btn-success" onclick="confirmacion_guardado()"><strong>{vtranslate('LBL_SAVE', $MODULE)}</strong></button>
				</div>
			</h4>
		</div>
		<hr>
		<div class="blockData">
			<table class="table detailview-table no-border">
				<tbody>
					<tr>
						<td> {include file=vtemplate_path("cytoscapeGrafica.tpl", 'LPTempFlujos')} </td>
					</tr>
				</tbody>
			</table>
		</div>
