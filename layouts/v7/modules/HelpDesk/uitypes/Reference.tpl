...
{assign var="ES_READONLY" value=$FIELD_NAME eq 'contact_id' && $HAY_LLAMADA_ACTIVA}
...
<div class="referencefield-wrapper {if $FIELD_VALUE neq 0} selected {/if}">
    ...
    <div class="input-group">
        ...
        <input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="reference" type="text" 
            ...
            {/if}
            />
        <a href="#" class="clearReferenceSelection {if $FIELD_VALUE eq 0 || $ES_READONLY}hide{/if}"> x </a>
            {*customizo la busqueda cuando el modulo es Accounts, por eso cambio la clase*}
            <span class="input-group-addon {if in_array('Accounts', $REFERENCE_LIST)}customRelatedPopup{else}relatedPopup{/if} cursorPointer {if $ES_READONLY}hide{/if}" tabindex="0" title="{vtranslate('LBL_SELECT', $MODULE)}">
                <i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_select" class="fa fa-search"></i>
                <span role="button"  class="sr-only">{vtranslate('LBL_SELECT', $MODULE)}</span>
            </span>
        {*si el modulo de referencia es Accounts => quito el btn de + *}
        {if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES) && !in_array('Accounts', $REFERENCE_LIST)}
            <span class="input-group-addon createReferenceRecord cursorPointer clearfix {if $ES_READONLY}hide{/if}" title="{vtranslate('LBL_CREATE', $MODULE)}">
