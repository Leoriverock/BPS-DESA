        ...
        <form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php"
            ...
            <div class="editViewHeader">
                <div class='row'>
                    <div class="col-lg-12 col-md-12 col-sm-12 ">
                        ...
                        {if $RECORD_ID neq ''}
                        <h4 class="editHeader"
                            title="{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} {$RECORD_STRUCTURE_MODEL->getRecordName()}">
                            {vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} -
                            {$RECORD_STRUCTURE_MODEL->getRecordName()}</h4>
                        {else}
                        <h4 class="editHeader">{vtranslate('LBL_CREATING_NEW', $MODULE)}
                            {vtranslate($SINGLE_MODULE_NAME, $MODULE)}</h4>
                        {/if}
                    </div>
                </div>
            </div>
            ...
            <div class="editViewBody">
                <div class="editViewContents">
                    ...
                    <input type="hidden" name="returnsearch_params"
            ...
            <div class='modal-overlay-footer clearfix'>
                <div class="row clearfix">
                    <div class='textAlignCenter col-lg-12 col-md-12 col-sm-12 '>
                        <button type='submit' class='btn btn-success saveButton'>{vtranslate('LBL_SAVE',
                            $MODULE)}</button>&nbsp;&nbsp;
                        <a class='cancelLink' href="javascript:history.back()" type="reset">{vtranslate('LBL_CANCEL',
                            $MODULE)}</a>
                    </div>
                </div>
            </div>
...
<script>
    ...
    jQuery.ajax({
            ...
            url: 'index.php?module=Users&action=getPermisos',
            dataType: "json",
            success: function (data) {
                //SI NO ES ADMIN HAGO ESTO
                console.log("la informacion recibida es: ");
                console.log(data);
                if (!data) {
                    ...
                    $("[name='selectprefgroup']").prop("disabled", false);
                    $('.keyword-input').prop("disabled", false);

                    $("input[type='text']:not(.keyword-input)").css({
                        'background-color': '#eee',
                        'color': '#888',
                        'cursor': 'not-allowed'
                    });
                    $("input[disabled], select[disabled], textarea[disabled], input[type='checkbox']").attr('title', 'Necesitas permisos de administrador para poder editar');
