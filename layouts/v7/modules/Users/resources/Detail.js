...
Vtiger_Detail_Js("Users_Detail_Js",{
	...
	registerSaveOnEnterEvent: function(editElement) {
		...
		if(fieldName != 'us_grupopref') return;
		jQuery("[name='us_grupopref'] [label='Usuarios']").remove();
        jQuery("[name='us_grupopref'] [label='Grupos']").prepend('<option value="">Sin asignar</option>');
		jQuery("[name='us_grupopref']").select2('destroy').select2();
	},
