{* Todos los posibles campos de los posibles modulos configurables sin importar el UIType que tengan *}
<input type="hidden" name="posibles_tcd_campo" value='{Vtiger_Functions::jsonEncode($POSIBLES_TCD_CAMPO)}' />
                                ...
                                {if $FIELD_MODEL->isEditable() eq true}
                                    ...
                                    {if $FIELD_MODEL->get('uitype') neq '83'}
                                        <td class="fieldValue" {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if}
                                            ...
                                            {* *********************** *}
                                            {* {if $FIELD_MODEL->get('label') eq "tcd_template"} *}
                                                {* TRANSFORMAR EN PICKLIST *}
                                                {* <select name="tcd_template" class="inputElement select2"
                                                    data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="picklist"
                                                    data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                                                    {foreach from=$POSIBLES_TCD_TEMPLATE item=TCD_TEMPLATE}
                                                        <option value="{$TCD_TEMPLATE['lptempcamposid']}" data-modulo="{$TCD_TEMPLATE['tc_modulo']}"
                                                            {if $FIELD_MODEL->get('fieldvalue') eq {$TCD_TEMPLATE['lptempcamposid']}} selected {/if}
                                                            data-campo="{$TCD_TEMPLATE['tc_campo']}">
                                                            {vtranslate($TCD_TEMPLATE['tc_nombre'], $MODULE)}
                                                        </option>
                                                    {/foreach}
                                                </select> *}
                                            {* {else} *}
