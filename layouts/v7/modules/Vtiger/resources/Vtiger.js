...
Vtiger.Class('Vtiger_Index_Js', {
	...
	enviarPrefGroup: function(params, callback){
	...
	setEventGroupPref: function(){
		jQuery('#selectprefgroup').submit((e) => {
			e.preventDefault();
			var form = jQuery(e.currentTarget);
			var params = {
				'module' : 'Vtiger',
				'action' : 'GroupPref',
				'record' : jQuery('[name="userid"]', form).val(),
				'groupid' : jQuery('[name="selectprefgroup"]', form).val()
			}
			Vtiger_Index_Js.enviarPrefGroup(params, (data, err) => {
				if(!err){
					app.helper.showSuccessNotification({"message":'Se ha cambiado el grupo exitosamente'});
					jQuery('[name="prefgroup_label"]').text(jQuery('[name="selectprefgroup"] option:selected', form).text());
					localStorage.setItem('grupoPreflbl'+_USERMETA.id, jQuery('[name="selectprefgroup"] option:selected', form).text());
					localStorage.setItem('grupoPref'+_USERMETA.id, jQuery('[name="selectprefgroup"]', form).val());
					$('.grupopreferencia .dropdown-toggle').trigger('click');
				}else{
				}
			});
		});
		$(document).on('click', '.dropdown-menu.dropdownsegComen', function (e) {
			e.stopPropagation();
		});
		$('.grupopreferencia .dropdown-toggle').on('click', function() {
			var idgrupo = localStorage.getItem('grupoPref'+_USERMETA.id);
			var form = jQuery('#selectprefgroup');
			jQuery('[name="selectprefgroup"]', form).val(idgrupo);
			jQuery('[name="selectprefgroup"]', form).select2('destroy').select2();
		});
	}
	...
	registerEvents: function() {
		...
		if(localStorage.getItem('cwidmodal')){
			var cwid = localStorage.getItem('cwidmodal');
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					if(typeof ConsultasWeb_Detail_Js === 'function'){
                        if(cwid+'' == ConsultasWeb_Detail_Js.getInstance().getRecordId()+''){
                        	location.reload();
                        }
                    }						
				},
		...
		if(localStorage.getItem('numeroLlamado')){
			console.log('rollback presencial');
			jQuery.ajax({
				async: false,
				data: {
					modo:'liberarNumeroLlamado'
				},
				url:  'index.php?module=AtencionPresencial&action=getAcciones',
				dataType:"json",
				success: function(data) {
		...
		function relojGrupos(){
			setTimeout(() => {
				if(localStorage.getItem('grupoPreflbl'+_USERMETA.id)){
					if(jQuery('[name="prefgroup_label"]').text() != localStorage.getItem('grupoPreflbl'+_USERMETA.id)) console.log('cambiazo');
					jQuery('[name="prefgroup_label"]').text(localStorage.getItem('grupoPreflbl'+_USERMETA.id));
				}
				relojGrupos()
			}, 5000);
		}

		if(jQuery('[name="prefgroup_label"]').data('name')) {
			localStorage.setItem('grupoPreflbl'+_USERMETA.id, jQuery('[name="prefgroup_label"]').data('name'))
			localStorage.setItem('grupoPref'+_USERMETA.id, jQuery('[name="prefgroup_label"]').data('id'))
			setTimeout(() => {relojGrupos();},2000);
		}
	},
