		...
		{if $MODULE_NAME ne 'Leads'}
			<div class="col-lg-5 commentHeader pull-right" style="margin-top:5px;text-align:right;padding-right:20px;">
				...
				<input type="checkbox" class="bootstrap-switch pull-right" id="rollupcomments" hascomments="1" startindex="{$STARTINDEX}" data-view="summary" rollupid="{$ROLLUPID}" 
					rollup-status="{$ROLLUP_STATUS}" module="{$MODULE_NAME}" record="{$PARENT_RECORD}" data-on-color="success"/>
			</div> 
		{/if}
	</div>
									...
									<div class="media">
										...
										<div class="media-body" style="width:100%">
											<div class="comment" style="line-height:1;">
												...
												{*Para mostrar el enlace a la llamada si corresponde*}
												{if $COMMENT->get('callsid')}
													<span class="pull-right">
													{assign var="MODULERELAT" value=Vtiger_Functions::getCRMRecordType($COMMENT->get('callsid'))}
														<i class="fa fa-{if $MODULERELAT ==='Calls'}phone{else}globe{/if}" data-toggle="tooltip" data-placement="top" 
															data-original-title="Comentario creado durante la {if $MODULERELAT ==='Calls'}llamada{else if $MODULERELAT === 'AtencionesWeb'}atención web{else}atención presencial{/if}"></i>&nbsp;
														{assign var="FECHA_HORA" value=$COMMENT->getDatosLlamada()}
														<small title="Ir a la {if $MODULERELAT ==='Calls'}llamada{else}atención web{/if}">
														<a href="index.php?module={$MODULERELAT}&view=Detail&record={$COMMENT->get('callsid')}" target="_blank">{$FECHA_HORA}</a>
														</small>
													</span>
												{/if}
