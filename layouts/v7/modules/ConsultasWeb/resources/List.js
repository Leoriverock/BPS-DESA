...
Vtiger.Class("ConsultasWeb_List_Js", {
	listInstance: false,
	filterClick: false,
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
    ...
    registerAutoIncludeFieldsInMassEdit: function () {
    	...
    	var autoIncludeFieldsInMassEditCallback = function() {
			...
			//Redmine Defecto #167403 29/09/23
			//$(this).closest('tr').find("input[id=include_in_mass_edit_" + fieldName + "]").prop( "checked", true );
			var campoCwGrupo = $("td.fieldValue select[name='cw_grupo']");
			
			campoCwGrupo.on("click", function() {
				console.log(campoCwGrupo);
				$(this).closest('tr').find("input[id=include_in_mass_edit_" + fieldName + "]").prop("checked", true);
			});
			if (fieldName !== "cw_grupo" ) {
	...
	saveMassEdit: function (event) {
		...
		var changeoptionvalue = 0;
		var cw_persona = $('input[name="cw_persona"]').val(); 
		var cw_tema = $('input[name="cw_tema"]').val(); 
		var cw_estado = $('select[name="cw_estado"]').val(); 
		var cw_grupo = $('select[name="cw_grupo"]').val(); 
		var	assigned_user_id = $('select[name="assigned_user_id"]').val(); 
	
		//Paso 1: traer todas las consultas a editar
		//Paso 2: Verificar si cuando guardo solo estado = "pendiente", alguna de las consultas no tenga asignado un usuario
		//Paso 3: Verificar si cuando guardo solo estado = "pendiente agente", alguna de las consultas no tenga asignado un grupo
		//Paso 4: Verificar que si cambio el "asignado a" no quede asociado a un grupo que no corresponda
		
		var checkboxes = $('.inputElement[type="checkbox"]');
		...
		checkboxes.each(function() {
			if ($(this).is(':checked')) {
				...
				if (
					(checkboxId === 'include_in_mass_edit_cw_persona' && cw_persona === '') ||
					(checkboxId === 'include_in_mass_edit_cw_tema' && cw_tema === '') ||
					(checkboxId === 'include_in_mass_edit_cw_estado' && cw_estado === '') ||
					(checkboxId === 'include_in_mass_edit_assigned_user_id' && assigned_user_id === '') ||
					(checkboxId === 'include_in_mass_edit_cw_grupo' && cw_grupo === '')
				) {
					changeoptionvalue = 1;
				}
			}
			...
			console.log("changeoptionvalue"+changeoptionvalue);
		

		
		
		app.helper.showProgress();
		if (changedFields.length > 0 && changeoptionvalue == 0) {
