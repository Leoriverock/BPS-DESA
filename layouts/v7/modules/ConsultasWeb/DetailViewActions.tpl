...
{strip}
    <div class="col-lg-6 detailViewButtoncontainer">
        <div class="pull-right btn-toolbar">
            ...
            {assign var=acceso value=$RECORD->tieneAcceso()}
            {assign var=pertenece value=$RECORD->perteneceGrupo($RECORD->get('cw_grupo'))}
            {if $acceso } <!-- && pertenece -->
                {if $RECORD->get('cw_estado') == 'Pendiente' or $RECORD->get('cw_estado') == 'Pendiente Agente'}
                <input id="asignarAtenderBtn" class="btn btn-success" onclick="ConsultasWeb_Detail_Js.cargarModal({$RECORD->get('id')})" value="Asignar/Atender">
                {/if}
            {/if}
            {*var_dump($RECORD->get('cw_estado'))*}
...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        console.log(estado);
        if (estado === 'Asignado') {
                $(this).find('.action.pull-right').hide();
            }

            $("#asignarAtenderBtn").click(function() {
                var boton = $(this);
                
                // Verificar si el botón no está deshabilitado (evitar doble clic)
